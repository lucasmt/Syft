...
OldDFA::OldDFA(shared_ptr<Cudd> m)
  : mgr(move(m))
{}
...
void OldDFA::initialize(string filename, string partfile){
    //ctor
...
void OldDFA::read_partfile(string partfile){
    ifstream f(partfile);
...
void OldDFA::read_from_file(string filename){
ifstream f(filename);
	if(f.is_open()){
		bool flag = 0;
		string line;
...
void OldDFA::construct_from_comp_front(string filename){
...
void OldDFA::construct_from_comp_back(vbdd& S2S, vbdd& S2P, vbdd& Svars, vbdd& Ivars, vbdd& Ovars, std::vector<int> IS){
...
void OldDFA::recur(int index, item tmp){
    if(smtbdd[index][0] == -1){
...
void OldDFA::recur_left(int index, item tmp, int v){
...
void OldDFA::recur_right(int index, item tmp, int v){
	while(tmp.size() < (nbits+v))//
		tmp.push_back(2); //0:false 1:true 2:don't care
	tmp.push_back(1);
	recur(index, tmp);
...
void OldDFA::push_states(int i, item & tmp){
...
void OldDFA::print( vector <string> & v )
{
...
void OldDFA::print_int( vector <int> & v )
{
  for (size_t n = 0; n < v.size(); n++)
    cout<< v[ n ] << " ";
  cout << endl;
...
void OldDFA::print_vec(vector<item> & v){
    for (size_t n = 0; n < v.size(); n++)
        print_int(v[n]);
  cout << endl;
}

bool OldDFA::strfind(string str, string target){
...
BDD OldDFA::var2bddvar(int v, int index){
    if(v == 0){
...
void OldDFA::dumpdot(BDD &b, string filename){
